...
export async function create(req:Request,res:Response){
  ...
  if(await connection("users").where("email",email).first())return res.status(400).send({message:"User already exists"})
...
export async function auth(req:Request,res:Response){
  ...
  try{
    ...
    if(!await bcryptjs.compare(password,user.password)){//verifica se a senha existe no banco de dados
